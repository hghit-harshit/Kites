...
public:
	...
	const std::map<int, std::string>& GetLineInstructionTypes() const { return line_to_instruction_types_mapping_; }
...
private:
	...
	std::map<unsigned int, std::string> instruction_index_to_mnemonics_;  // PC/instruction index -> mnemonic
	std::map<int, int> line_to_execution_counts_; // line number -> execution count
	std::map<int, std::string> line_to_instruction_types_mapping_;  // line number -> instruction type
};