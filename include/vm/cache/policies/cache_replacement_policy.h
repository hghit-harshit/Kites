...
/**
 * @brief This structure provides contextual information about the cache state for replacement policies.
 * Ths is basically a snapshot of the cache configuration.
 * 
 */
...
public:
    ...
    /**
     * @brief Given the current state of the cache lines in a set, the incoming request, and the overall cache context, choose a victim line index for eviction.
     * 
    ...
    virtual void onAccess(const CacheLineView& line,
                          const CacheRequestView& request,
                          const CacheContextView& context) = 0;

    virtual void onInsert(const CacheLineView& line,
                          const CacheRequestView& request,
                          const CacheContextView& context) = 0;

    virtual void onEvict(const CacheLineView& line,
                         const CacheRequestView& request,
                         const CacheContextView& context) = 0;
