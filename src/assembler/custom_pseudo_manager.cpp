...
std::string CustomPseudoManager::expandPseudoInstruction(const std::string& source)
{
    ...
    {   
        ...
        const QStringList tokens = trimmedLine.split(QRegularExpression("\\s+"), Qt::SkipEmptyParts);
        ...
        {
            QStringList expansionLineTokens = expansionLineval.toString().split(QRegularExpression("\\s+"), Qt::SkipEmptyParts);
...
bool CustomPseudoManager::isValidExpainsionLine(const QString &line, const QStringList& declaredArgs, QString &errorMessage)
{
    ...
    const QString opcode = parts.first().toLower();
    ...
    const int operandCount = parts.size() - 1;
    ...
    {
        const QString operand = parts[i + 1];
