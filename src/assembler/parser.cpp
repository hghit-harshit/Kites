/**
 * @file parser.cpp
 * @brief Contains the implementation of the Parser class for parsing tokens and generating
 * intermediate code.
...
void Parser::parseDataDirective()
{
    ...
    while (currentToken().value != "text" && currentToken().value != "data" &&
           currentToken().value != "bss" && currentToken().value != "section" &&
           currentToken().type != TokenType::EOF_)
    {
        ...
        if (currentToken().type == TokenType::LABEL)
        {
            ...
            else
            {
                ...
                errors_.all_errors.emplace_back(errors::SyntaxError(
                    "Invalid directive",
                    "Expected .dword, .word, .halfword, .byte, .float, .double, .string, .zero",
        ...
        if (currentToken().value == "dword")
        {
            ...
            while (
                currentToken().type != TokenType::EOF_ &&
                (currentToken().type == TokenType::NUM || currentToken().type == TokenType::COMMA))
            {
                if (currentToken().type == TokenType::NUM)
                {
                    align(8);
                    data_buffer_.emplace_back(
                        static_cast<uint64_t>(std::stoull(currentToken().value)));
                    data_index_ += 8;
                }
                nextToken();
            }
        ...
        else if (currentToken().value == "word")
        {
                ...
                (currentToken().type == TokenType::NUM || currentToken().type == TokenType::COMMA))
            {
                if (currentToken().type == TokenType::NUM)
                {
                    align(4);
                    data_buffer_.emplace_back(
                        static_cast<uint32_t>(std::stoull(currentToken().value)));
                    data_index_ += 4;
                }
                nextToken();
            }
        ...
        else if (currentToken().value == "halfword")
        {
                ...
                (currentToken().type == TokenType::NUM || currentToken().type == TokenType::COMMA))
            {
                if (currentToken().type == TokenType::NUM)
                {
                    align(2);
                    data_buffer_.emplace_back(
                        static_cast<uint16_t>(std::stoull(currentToken().value)));
                    data_index_ += 2;
                }
                nextToken();
            }
        ...
        else if (currentToken().value == "byte")
        {
                ...
                (currentToken().type == TokenType::NUM || currentToken().type == TokenType::COMMA))
            {
                if (currentToken().type == TokenType::NUM)
                {
                    align(1);
                    data_buffer_.emplace_back(
                        static_cast<uint8_t>(std::stoull(currentToken().value)));
                    data_index_ += 1;
                }
                nextToken();
            }
                ...
                (currentToken().type == TokenType::NUM || currentToken().type == TokenType::COMMA))
            {
                if (currentToken().type == TokenType::NUM)
                {
                    ...
                    else
                    {
                        ...
                        errors_.all_errors.emplace_back(errors::SyntaxError(
                            "Invalid zero directive", "Expected a positive number", filename_,
        ...
        else
        {
            ...
            errors_.all_errors.emplace_back(errors::SyntaxError(
                "Invalid directive",
                "Expected .dword, .word, .halfword, .byte, .string, .float, .double, .zero",
                filename_, currentToken().line_number, currentToken().column_number,
                GetLineFromFile(filename_, currentToken().line_number)));
            nextToken();
        }
...
void Parser::parseTextDirective()
{
    ...
    while (currentToken().type != TokenType::DIRECTIVE && currentToken().type != TokenType::EOF_)
    {
        ...
        if (currentToken().type == TokenType::LABEL)
        {
            if (symbol_table_.find(currentToken().value) != symbol_table_.end())
            {
                ...
                recordError(ParseError(
                    ...
                    "Label redefinition: already defined at line " +
                        std::to_string(symbol_table_[currentToken().value].line_number)));
                errors_.all_errors.emplace_back(errors::LabelRedefinitionError(
                    "Label redefinition",
            ...
            symbol_table_[currentToken().value] = {instruction_index_ * 4,
                                                   currentToken().line_number, false};
            nextToken();
        }
        else if (currentToken().type == TokenType::OPCODE)
        {
            std::vector<instruction_set::SyntaxType> syntaxes =
                instruction_set::instruction_syntax_map[currentToken().value];
        ...
        else
        {
            ...
            errors_.all_errors.emplace_back(errors::UnexpectedTokenError(
                "Unexpected token", filename_, currentToken().line_number,
...
void Parser::parseBSSDirective()
{
           ...
           currentToken().type != TokenType::EOF_)
    {
        ...
        //     nextToken();
        //     if (currentToken().type==TokenType::NUM) {
        //       symbol_table_[currentToken().value] = {data_index_, currentToken().line_number,
        //       true}; data_index_ += std::stoull(currentToken().value); nextToken();
        //     } else {
        //       errors_.count++;
        //       recordError(ParseError(currentToken().line_number,
...
void Parser::parse()
{
    ...
    while (currentToken().type != TokenType::EOF_)
    {
                  ...
                  currentToken().type == TokenType::OPCODE))
        {
            while (currentToken().type != TokenType::EOF_ && currentToken().value != "data" &&
                   currentToken().value != "section" && currentToken().value != "bss")
            {
                nextToken();
            }
        }
        ...
        else
        {
            ...
            errors_.all_errors.emplace_back(errors::SyntaxError(
                "Invalid token", "Expected: .data, .text, or <opcode> or <label>", filename_,
                currentToken().line_number, currentToken().column_number,
                GetLineFromFile(filename_, currentToken().line_number)));
            nextToken();
        }
    ...
    while (currentToken().type != TokenType::EOF_)
    {
        ...
        else
        {
            ...
            errors_.all_errors.emplace_back(errors::SyntaxError(
                "Invalid token", "Expected: .data, .text, .bss or <opcode> or <label>", filename_,
                currentToken().line_number, currentToken().column_number,
                GetLineFromFile(filename_, currentToken().line_number)));
            nextToken();
        }
        ...
        if (symbol_table_.find(block.getLabel()) != symbol_table_.end())
        {
            ...
            if (instruction_set::isValidBTypeInstruction(block.getOpcode()))
            {
                if (!symbol_table_[block.getLabel()].isData)
                {
                    ...
                    else
                    {
                        ...
                        errors_.all_errors.emplace_back(errors::ImmediateOutOfRangeError(
                            "Immediate value out of range", "Expected: -4096 <= imm <= 4095",
                            filename_, block.getLineNumber(), 0,
                ...
                else
                {
                    ...
                    errors_.all_errors.emplace_back(errors::InvalidLabelRefError(
                        "Invalid label reference", "Label references data", filename_,
            ...
            else
            {
                ...
                recordError(ParseError(block.getLineNumber(),
                                       "Invalid label reference: Label references data"));
                errors_.all_errors.emplace_back(errors::InvalidLabelRefError(
                    "Invalid label reference", "Label reference not found", filename_,
                    block.getLineNumber(), 0, GetLineFromFile(filename_, block.getLineNumber())));
                continue;
            }
