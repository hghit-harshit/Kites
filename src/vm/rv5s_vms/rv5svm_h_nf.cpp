 ...
 * * Stall Rule: Any GPR dependency results in a 2-cycle stall (2 bubbles).
...
#include "vm/rv5s_vms/rv5s_hdu.h" 
...
void RV5StageVM_H_NF::pipeline_decode()
{
    ...
    // Read GPR register data naively (NO FORWARDING - Data will be correct due to stall)
    ...
    // FPR write enable is not needed for a pure integer simulator, so it's omitted.
...
void RV5StageVM_H_NF::pipeline_execute()
{
    ...
    // FPR results are omitted for pure integer
    ex_mem_reg_.rd = id_ex_reg_.rd; 

    ex_mem_reg_.reg2_data = id_ex_reg_.reg2_data; // Data for Store
    
    // Pass control signals...
    ex_mem_reg_.reg_write = id_ex_reg_.reg_write;
    // FPR write enable is omitted
...
void RV5StageVM_H_NF::Undo()
{
    ...
    {
        // Only GPR writeback logic remains
