...
CompilerTab::CompilerTab(QWidget *parent) : KitesTab(parent), ui(new Ui::CompilerTab)
{
    ui->setupUi(this);
    connect(ui->convertToAssemblyButton, &QPushButton::clicked, this,
...
void CompilerTab::onConvertToAssemblyClicked()
{
    ...
    QJsonObject options;
    options["userArguments"] = "-march=rv64gc -mabi=lp64d "
                               "-fno-exceptions "
                               "-fno-asynchronous-unwind-tables "
                               "-fno-unwind-tables "
                               "-fno-ident " +
                               getUsersCompilerOptions();
    ...
    connect(
        ...
        [this, reply, manager]()
        {
            ...
            {
                ui->outputTextEdit->setPlainText("Network error: " + reply->errorString() +
            ...
            if (exitCode != 0)
            {
                ui->outputTextEdit->setPlainText("Compilation failed (exit code " +
...
QString CompilerTab::cleanAssembly(const QString &rawAssembly)
{
    ...
    for (QString line : lines)
    {
        ...
        if (trimmed.startsWith(".debug_") || trimmed.startsWith(".Ldebug") ||
            trimmed.startsWith(".Ltext") || trimmed.startsWith(".Letext") ||
            trimmed.startsWith(".LFB") || trimmed.startsWith(".LFE") || trimmed.startsWith(".LVL"))
            continue;

        // Skip debug/meta directives
        if (trimmed.startsWith(".file") || trimmed.startsWith(".loc ") ||
            trimmed.startsWith(".cfi_") || trimmed.startsWith(".option") ||
            trimmed.startsWith(".attribute") || trimmed.startsWith(".ident") ||
            trimmed.startsWith(".size") || trimmed.startsWith(".uleb128") ||
            trimmed.startsWith(".sleb128") || trimmed.startsWith(".4byte") ||
            trimmed.startsWith(".2byte") || trimmed.startsWith(".byte") ||
            trimmed.startsWith(".8byte") || trimmed.startsWith(".string") ||
            trimmed.startsWith(".LASF") || trimmed.startsWith(".note"))
            continue;
