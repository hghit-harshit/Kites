...
#include "processor/cache/policies/cache_replacement_policy.h"
...
class CustomPolicyEngine
{
  public:
    ...
    size_t callChooseVictim(std::span<const CacheLineView> cacheLines,
                            const CacheRequestView &request, const CacheContextView &context);

    void callOnAccess(std::span<const CacheLineView> cacheLines, const CacheRequestView &request,
                      const CacheContextView &context);

    void callOnInsert(std::span<const CacheLineView> cacheLines, const CacheRequestView &request,
                      const CacheContextView &context);

    void callOnEvict(std::span<const CacheLineView> cacheLines, const CacheRequestView &request,
                     const CacheContextView &context);
  ...
  private:
    ...
    // These push functions convert the cache state into Lua tables and push them onto the Lua stack
    // for the custom policy functions to consume
